...
class ProfileForm(forms.Form):
    ...
    # List of field names that will be excluded in addition
    # to default_field_excludes(if active)
    excluded_fields = []
    ...
    dob_year = forms.ChoiceField(
        label=_("Birth Year"),
        choices=([('', 'Year'), ] + [(i, i) for i in \
                reversed(
                    range(datetime.now().year - 100,
                    datetime.now().year - 4))]
                ),
        required=False,
    )
    ...
    def _gen_dob(self):
        if 'dob_day' in self.cleaned_data and 'dob_month' in self.\
                cleaned_data and 'dob_year' in self.cleaned_data:
            if ['', '', ''] == [self.cleaned_data['dob_day'], self.\
                    cleaned_data['dob_month'], self.cleaned_data['dob_year']]:
                return None
            else:
                return date(
